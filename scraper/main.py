...
# After pruning seret to Planet-only titles, the stages are more balanced —
# planet_showtimes and planet_details each take as long as seret.
_STAGE_WEIGHTS = {
    "idle": 0.0,
    "planet_dates": 0.02,
...
def run_scrape() -> None:
    ...
    try:
        ...
        # Pass Planet's titles so seret only fetches the ~25 matching films,
        # not all 120+ from the discovery pages. Combined with the 12h
        # detail-page cache, this cuts the scrape from ~3 min to ~20-30 s
        # on a fresh run and to seconds on subsequent runs.
        from scraper.seret import scrape_movies

        planet_titles = [pf.name for pf in planet_films if pf.name]
        ...
        seret_movies = scrape_movies(
            planet_titles=planet_titles,
            progress=_seret_progress,
        )
        ...
        # Warm the poster cache in the background so the first visitor after
        # a scrape doesn't eat the upstream-fetch latency. We don't block
        # "done" on it — any cache misses still work via /poster/<id>.
        def _prefetch():
            try:
                from web.app import prefetch_posters
                prefetch_posters()
            except Exception as e:
                logger.warning("poster prefetch skipped: %s", e)

        import threading as _threading
        _threading.Thread(target=_prefetch, daemon=True, name="poster-prefetch").start()
